class StudyPlanner {
    ...
    handleFormSubmit(e) {
        ...
        const invalidDates = subjects.filter(subject => {
            const examDate = new Date(subject.examDate);
            return examDate <= today;
        });

        if (invalidDates.length > 0) {
            alert(`Please select future exam dates for: ${invalidDates.map(s => s.name).join(', ')}`);
            return;
        }
        ...
        const schedule = {
            ...
            tasks: this.generateStudyPlan(subjects, dailyHours)
        };
    ...
    getSubjectsFromForm() {
        ...
        subjectInputs.forEach(input => {
            ...
            const examDate = input.querySelector('.exam-date').value;
    ...
    generateStudyPlan(subjects, dailyHours) {
        const tasks = [];
        const daysOfWeek = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'];
        const today = new Date();
        
        // Calculate days until exam and priority scores for each subject
        const subjectsWithPriority = subjects.map(subject => {
            ...
            return {
                ...
                priorityScore: subject.difficulty * (10 / Math.max(daysUntilExam, 1)), // Higher priority for closer exams
    ...
    displaySchedule(schedule) {
        ...
        // Get unique subjects with their exam dates for the title
        const uniqueSubjects = [...new Set(schedule.subjects.map(s => s.name))];
        const subjectCount = uniqueSubjects.length;
        
        document.getElementById('planTitle').textContent = `${schedule.title} (${subjectCount} subject${subjectCount > 1 ? 's' : ''})`;
        ...
        // Add subject overview with exam dates
        const subjectOverviewDiv = document.createElement('div');
        subjectOverviewDiv.className = 'subject-overview';
        subjectOverviewDiv.innerHTML = `
            <h3>Subject Exam Dates</h3>
            <div class="subject-dates">
                ${schedule.subjects.map(subject => {
                    ...
                    return `
                        <div class="subject-date-item difficulty-${this.getDifficultyName(subject.difficulty)}">
        ...
        `;
        planContent.appendChild(subjectOverviewDiv);
            ...
            dayTasks.forEach(task => {
                ...
                taskDiv.innerHTML = `
                    <div class="task-info">
                        ...
                        <div class="task-exam-info">Exam: ${dateStr} (${task.daysUntilExam} day${task.daysUntilExam !== 1 ? 's' : ''})</div>
                        <div class="task-priority">Priority Score: ${task.priorityScore.toFixed(1)}</div>
                    </div>
    ...
    editCurrentSchedule() {
        ...
        schedule.subjects.forEach(subject => {
            ...
            subjectDiv.innerHTML = `
                ...
                <input type="date" class="exam-date" value="${subject.examDate}" required title="Exam date for this subject">
                <select class="difficulty-level">
